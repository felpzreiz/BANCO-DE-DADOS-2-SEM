...
public class Operacoes {
    ...
    public String simplificar(int newNumerador, int newDenominador){
        int menor = 1;
        if(Math.abs(newNumerador) > Math.abs(newDenominador)){
            menor = newDenominador;
        } else if (Math.abs(newNumerador) < Math.abs(newDenominador)) {
            menor = newNumerador;
        }else{
            return("1/1");
        }
        ...
        for(int i = menor; i >= 2; i--){
            if((Math.abs(newNumerador)%i == 0) && (Math.abs(newDenominador)%i == 0)){

                System.out.println(menor);
                return (newNumerador/i + "/" + newDenominador/i);

            } else if ((i==2) && ((newNumerador%i != 0) || (newDenominador%i != 0))){
